 ...
 * @author Julen
 */
public class confFotoMIBServlet extends HttpServlet {
           ...
           Part part;
            ...
            if (part.isParam()) // es de tipo 'loqueseamenosfile'?? ein??
            {
                ...
                if(!fin)
                {
                    ...
                    if(n.equals("nombre"))
                        nombre=valorcadena;
                    if(n.equals("comentario"))
                        comentario=valorcadena;
                    if(n.equals("enviar"))
                        if(valorcadena.equals("Alta"))
                        {
                            if(fotoDAL.insertarFoto(nombre,url,comentario))
                            {
                            ...
                            if (valorcadena.equals("Eliminar"))
                            {
                                ...
                                Foto f = (Foto)session.getAttribute("fotoDetalle");
                                //Foto f=fotoDAL.getFoto(id);
                               if(f.getIdFoto()!=1)
                               {
                               String[] archivos = f.getUrl().split("/");
                               String dirUploadFiles;
                               ServletContext sc = request.getSession().getServletContext();
                               dirUploadFiles = sc.getRealPath("imagenes");
                               File fich=new File(dirUploadFiles,archivos[1]);
                               fich.delete();

                                    SocioDAL socioDAL = new SocioDAL();
                                    socioDAL.quitarFoto(f.getIdFoto());
                                    //fotoDAL.quitarFoto(String.valueOf(f.getIdFoto()));
                                    if(fotoDAL.eliminarFoto(String.valueOf(f.getIdFoto())))
                                    {
                            ...
                            if (valorcadena.equals("Modificar"))
                            {
                                ...
                                if(f.getIdFoto()!=1)
                                {
                                    ...
                                    if(fotoDAL.modificarFoto(String.valueOf(f.getIdFoto()), nombre, comentario))
                                    {
            ...
            else if (part.isFile()) //es el fichero??
            {
                ...
                String nombreFichero = filePart.getFileName();
                 // Obtener la extension
                    ...
                    {
                        resultado = "El archivo de la foto debe ser .gif o .jpg";
                        request.getSession().setAttribute("resultado", resultado);
