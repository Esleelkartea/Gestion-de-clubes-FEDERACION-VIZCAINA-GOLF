 ...
 * @author Julen
 */
public class eCursoInscritoMIBServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try
        {
            ...
            Curso unCurso = (Curso)request.getSession().getAttribute("unCurso");

            CursoInscritoDAL cursoInscritoDAL = new CursoInscritoDAL();
            CursoDAL cursoDAL = new CursoDAL();
            cursoDAL.sumarAsistente(unCurso.getCursoId());
            if(cursoInscritoDAL.insertarCursoInscritos(unSocio.getIdSocio(), unCurso.getCursoId()))
            {
        ...
        {
            getServletContext().getRequestDispatcher("/eCursosInscritos2.jsp").forward(request, response);
            out.close();
        }
    } 
